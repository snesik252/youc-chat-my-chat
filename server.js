...
app.post('/api/register', async (req, res) => {
  try {
    ...
    await supabase.from('users').insert([
      { username, passwordHash, friends: [], groups: [], avatar: '', description: '' }
    ]);
...
app.get('/api/friend-requests', async (req, res) => {
  try {
    ...
    console.error('/api/friend-requests error:', e);
    res.status(500).json({ error: 'Ошибка сервера' });
  }
});
...
app.post('/api/send-friend-request', async (req, res) => {
  try {
    ...
    await supabase.from('friend_requests').insert([
      { requestId: Date.now().toString(), from, to, status: 'pending' }
    ]);

    res.json({ success: true });
  } catch (e) {
    console.error('/api/send-friend-request error:', e);
    res.status(500).json({ error: 'Ошибка сервера' });
  }
});
...
app.post('/api/handle-friend-request', async (req, res) => {
  try {
    ...
    console.error('/api/handle-friend-request error:', e);
    res.status(500).json({ error: 'Ошибка сервера' });
  }
});
...
io.on('connection', (socket) => {
  ...
  socket.on('send-friend-request', async (to) => {
    ...
    if (toSock) {
      io.to(toSock).emit('new-notification', {
        id: Date.now(),
    ...
    if (fromSock) {
      ...
      if (fromUser) {
        io.to(fromSock).emit('user-data', {
          ...
          groups: fromUser.groups,
          avatar: fromUser.avatar
        });
      }
    }

    // Обновить данные у получателя
    const { data: currentUser } = await supabase.from('users').select('*').eq('username', username).maybeSingle();
    socket.emit('user-data', {
      name: username,
  ...
  socket.on('create-group', async (groupData) => {
    ...
    for (const member of allMembers) {
      ...
      const sockId = userSockets.get(member);
      if (sockId) {
        const { data: updatedUser } = await supabase.from('users').select('*').eq('username', member).maybeSingle();
        if (updatedUser) {
