...
const supabaseUrl = 'https://oafwaofiuczljckmxmko.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9hZndhb2ZpdWN6bGpja214bWtvIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3ODA2NTYwNCwiZXhwIjoyMDkzNjQxNjA0fQ.ZItQzDwbrZaXyfCs_y5ZuMgDrvwBObBzIK6JB4DjnI4';
...
// Гарантированное создание/обновление тестового аккаунта
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (!bcrypt.compareSync(password, user.passwordHash)) { console.log('❌ Пароль не совпадает'); return res.status(401).json({ error: 'Неверный ник или пароль' }); }
...
app.post('/api/send-friend-request', async (req, res) => {
  ...
  if (toSock) io.to(toSock).emit('new-notification', { id: Date.now(), text: `${from} хочет добавить вас в друзья`, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), read: false });
  res.json({ success: true });
});
...
io.on('connection', (socket) => {
  ...
  socket.on('register', async (username) => {
    ...
    socket.emit('user-data', { name: username, friends: user.friends, groups: user.groups, avatar: user.avatar, description: user.description });
    broadcastUsersList();
  });
  ...
  socket.on('chat message', async (msgData) => {
    ...
    if (isGroup) {
      ...
      group.members.forEach(member => { const sockId = userSockets.get(member); if (sockId) io.to(sockId).emit('group message', { group: target, msg }); });
  ...
  socket.on('get-history', async (chatInfo) => {
    ...
    else { const myName = onlineUsers.get(socket.id); chatKey = [myName, target].sort().join(':'); }
  ...
  socket.on('respond-friend-request', async ({ requestId, action }) => {
    ...
    } else { await supabase.from('friend_requests').update({ status: 'rejected' }).eq('requestId', requestId); }
    const fromSock = userSockets.get(request.from);
    if (fromSock) { const { data: fromUser } = await supabase.from('users').select('*').eq('username', request.from).maybeSingle(); if (fromUser) io.to(fromSock).emit('user-data', { name: request.from, friends: fromUser.friends, groups: fromUser.groups, avatar: fromUser.avatar }); }
  ...
  socket.on('create-group', async (groupData) => {
    ...
    for (const member of allMembers) {
      ...
      if (user) { const updatedGroups = [...user.groups, name]; await supabase.from('users').update({ groups: updatedGroups }).eq('username', member); }
      const sockId = userSockets.get(member);
      if (sockId) { const { data: updatedUser } = await supabase.from('users').select('*').eq('username', member).maybeSingle(); if (updatedUser) io.to(sockId).emit('user-data', { name: member, friends: updatedUser.friends, groups: updatedUser.groups, avatar: updatedUser.avatar }); }
    }
  });
  ...
  socket.on('answer-call', ({ to, answer }) => { const receiverSocket = userSockets.get(to); if (receiverSocket) io.to(receiverSocket).emit('call-answered', { answer }); });
  socket.on('ice-candidate', ({ to, candidate }) => { const receiverSocket = userSockets.get(to); if (receiverSocket) io.to(receiverSocket).emit('ice-candidate', { candidate }); });
  socket.on('end-call', ({ to }) => { const receiverSocket = userSockets.get(to); if (receiverSocket) io.to(receiverSocket).emit('call-ended'); });
});
